...
class sdbh
{
    function __construct($force_master = false)
    {
        $this->port = 3306;
        $this->host = 'localhost';
        $this->dbname = 'f0848391_test';
        $this->user = 'f0848391_test';
        $this->pass = 'lrmsYbnk';
    ...
    public function get_info($tbl_name, $product_name, $days_count = False, $checkbox = False)
    {
        while (True) try {
            $query = $this->query_exc("SELECT * FROM `$tbl_name` WHERE `NAME` = '$product_name';");
            break;
        } catch (sdbh_tables_exception $e) {
            $query = false;
            break;
